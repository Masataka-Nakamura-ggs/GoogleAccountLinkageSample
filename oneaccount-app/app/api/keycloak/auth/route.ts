...
export async function GET(request: NextRequest) {
  console.log('=== oneaccount-app: /api/keycloak/auth GET request ===');
  
  // GMOコインアプリへのリダイレクトを行うための設定
  const gmoAppCallbackUrl = 'http://localhost:3000/api/keycloak/callback';
  
  // Keycloak認証URLの構築
  const keycloakUrl = 'http://localhost:8080/realms/one-account-realm/protocol/openid-connect/auth';
  const clientId = 'gmo-coin-client'; // GMOコインアプリと同じクライアントIDを使用
  const redirectUri = encodeURIComponent(gmoAppCallbackUrl);
